...
public class MarkScene1 implements Initializable
{
    ...
    public void mark()
    {
        ...
        for(int i=1;i<=5;i++)
        {
            ...
            switch (i)
            {
                case 1:Grade1.setText(STR."\{Judges.getJudgeList().get(i).getSumGrade()}"); theGrade+=Judges.getJudgeList().get(i).getSumGrade(); break;
                case 2:Grade2.setText(STR."\{Judges.getJudgeList().get(i).getSumGrade()}"); theGrade+=Judges.getJudgeList().get(i).getSumGrade(); break;
                case 3:Grade3.setText(STR."\{Judges.getJudgeList().get(i).getSumGrade()}"); theGrade+=Judges.getJudgeList().get(i).getSumGrade(); break;
                case 4:Grade4.setText(STR."\{Judges.getJudgeList().get(i).getSumGrade()}"); theGrade+=Judges.getJudgeList().get(i).getSumGrade(); break;
                case 5:Grade5.setText(STR."\{Judges.getJudgeList().get(i).getSumGrade()}"); theGrade+=Judges.getJudgeList().get(i).getSumGrade(); break;
            }
        }

        //给选手统计最终成绩
        PlayerList.getInstance().getPlayerArrayList().get(currentCompetitor-1).setGrade(theGrade);
    }
